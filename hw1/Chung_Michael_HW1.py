...
def calculate( data, ithAttribute):
    ...
    data_tup_str = zip(*rows)[ithAttribute-1]
    rows = (row.strip().split(',') for row in data)
    data_tup_str = zip(*rows)[ithAttribute-1]
    datalist = [float(i) for i in data_tup_str]
    
    noOfObjects = len(datalist)
    minValue = min(datalist)
    maxValue = max(datalist)
    mean = numpy.mean(datalist)
    standardDeviation = numpy.std(datalist)
    q1 = numpy.percentile(datalist, 25)
    median = numpy.percentile(datalist, 50)
    q3 = numpy.percentile(datalist, 75)
    iqr = (q3-q1)
